<template>
  ...
  >
    ...
    <p>
      Stay updated on new <span class="bold">{{ decodedLocation }}</span> properties for sale via our <span
        class="bold"
      >private Facebook group.</span>
    </p>
    <a
...
export default {
  ...
  computed: {
    locations() {
      return ["costa-almería", "costa-blanca", "costa-del-sol", "costa-cálida", "gran-canaria", "ibiza", "lanzarote", "mallorca", "menorca", "tenerife"];
    }
  },
  mounted() {
    if(!this.show) return
  ...
  methods: {
    ...
    parseLocationFromURL() {
      const url = decodeURIComponent(window.location.href) // Get URL
      for (let i = 0; i < this.locations.length; i++) {
        if (url.includes(this.locations[i])) {
          const location = this.locations[i];
          return this.getRegion(location)
        }
      }
    },
    generateFacebookGroupUrl(location) {
      return `https://www.facebook.com/groups/propertiesforsalein${location}`; // Insert location into Facebook group URL
    },
    closeBanner() {
...
.slide-in {
  ...
  /* I thought this was a good sliding speed */
}
...
@keyframes slide-in {
  ...
  100% {
    bottom: 0rem;
    /* Slide up the screen to the bottom */
  }
}
...
@media (min-width: 1024px) {
  ...
  @keyframes slide-in {
    ...
    100% {
      left: 2rem;
      /* Slide in to the current position at the left-hand corner */
    }
  }
}
</style>
